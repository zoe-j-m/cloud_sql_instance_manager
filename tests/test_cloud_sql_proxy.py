...
class TestCloudSqlProxy:
    ...
    def test_run_proxy(self, popen_mock):
        ...
        returned_pid = run_cloud_sql_proxy(self.proxy_path, test_fixtures.connection_name1, test_fixtures.port1, False )
        assert returned_pid == pid
        popen_mock.assert_called_with(['/a/path', f'-instances={test_fixtures.connection_name1}=tcp:{test_fixtures.port1}'], start_new_session=True)

        mock_popen.pid = pid2
        returned_pid2 = run_cloud_sql_proxy(self.proxy_path, test_fixtures.connection_name2, test_fixtures.port2, True )
        assert returned_pid2 == pid2
        popen_mock.assert_called_with(['/a/path', '-enable_iam_login', f'-instances={test_fixtures.connection_name2}=tcp:{test_fixtures.port2}'], start_new_session=True)
    ...
    def test_is_running(self, mock_process):
        ...
        assert check_if_proxy_is_running(123, test_fixtures.connection_name1) is not None
